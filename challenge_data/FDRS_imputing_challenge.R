...
for(year in 2010:2022){
  fwrite(data %>% filter(source!="I",year==.env$year) %>% select(-source) %>% mutate(kpi=gsub("_IP","",kpi)),paste(r"(C:\Users\dirk.merkhof\OneDrive - IFRC\Documents\git\fdrs_imputation_2.0\challenge_data\challenge_data_)",year,".csv",sep=""))
}
...
for(year in 2021:2021){
  ...
  fwrite(data_jun,paste(r"(C:\Users\dirk.merkhof\OneDrive - IFRC\Documents\git\fdrs_imputation_2.0\challenge_data\challenge_data_)",year,"_jun.csv",sep=""))
}
